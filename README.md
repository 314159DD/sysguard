...
Process-storm guard for Windows. It watches the process families that runaway tooling spawns (`powershell`, `conhost`, `node`, `cmd`), and when a family crosses its limit it kills exactly the processes that are provably dead weight: shells stuck in startup, console hosts whose owner is gone, and orphaned `cmd -> node` trees. Two PowerShell files, a WinForms GUI, no dependencies.
...
![sysguard GUI: process families against their limits, kill buttons, live log](docs/gui.png)
...
```
sysguard.bat                   GUI monitor: live families vs limits, kill buttons, dry-run toggle, auto-guard
sysguard-guard.bat             headless guard: 10 s loop, applies the rules only while a limit is crossed
powershell -File sysguard.ps1 -Scan       print what the rules WOULD kill right now, exit
...
25 tests run the rule engine against synthetic process tables: dead chains, protected wrappers, PID reuse, threshold edges, dry-run vs real kills (with `Stop-Process` mocked), and config overrides. CI runs them on `windows-latest` under both Windows PowerShell 5.1 and PowerShell 7, then does a `-Scan` dry run against the runner itself.
...
tests/rules.Tests.ps1         Pester suite
docs/gui.png
```
